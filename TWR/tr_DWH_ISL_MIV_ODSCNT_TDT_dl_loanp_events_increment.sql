...
and cde.DCDEDATE >= @p_dfrom and cde.DCDEDATE <= @p_dto
--and cde.DWH_STATE = 'A'
...
FROM (
	 ...
	 ods.ODS_MIV_CDR_I cdr_i
WHERE 1=1
and cdr_i.DWH_ETL_DT between @p_dfrom_ods and @p_dto_ods
and cdr_i.DWH_STATE = 'A'
AND cdr_i.ncdragrid = TM_CDR.ncdragrid
AND cdr_i.icdrpart = TM_CDR.icdrpart
AND cdr_i.dcdrexist = TM_CDR.dcdrexist;
...
select --cur_cde для досрочного гашения increment где архивные проводки сравниваются с инкрементальной таблицой сотавшегося долга
	cde.NCDEAGRID
	, cde.ICDEPART
	, cde.DCDEDATE
	, cda3.RootID
...
select --cur_cde для досрочного гашения increment где инкрементные проводки сравниваются с архивной таблицой сотавшегося долга
	cde.NCDEAGRID
	, cde.ICDEPART
	, cde.DCDEDATE
	, cda3.RootID
...
where
	...
	and cde.DWH_ETL_DT between @p_dfrom_ods and @p_dto_ods
	and cde.NCDEAGRID = cda3.ProlongID
	and cde.ICDEPART = cda3.PartNum
--	and cde.DWH_STATE = 'A'	
	and cda3.Close_Date >= @p_initial_date
...
select distinct
	cda3.RootID
into #cdh_doc_inc
from ods.ODS_MIV_CDH_DOC cdh_doc
...
where
	...
	and cda3.PRE_UR_FIZ_TYPE = @acc_type
	and cda3.Close_Date >= @p_initial_date
	and cdh_doc.DWH_ETL_DT between @p_dfrom_ods and @p_dto_ods
...
--filter cdh_doc
select cdh_doc.*
into #cdh_doc_inc_filt
from #cdh_doc_inc inc
inner join #cur_CDH_DOC cdh_doc
on inc.RootID = cdh_doc.RootID
----------------------------------------------------------

--объединяем оба случая изменений досрочного гашения
select distinct sub.* 
into #cur_cde
from 
(--sub 
select * from #cur_cde_inc_1
union 
select * from #cur_cde_inc_2
) sub
--------------------------------
--select * from #cur_cde
----------------------------------------------------------
---********************************************************
---Completive select which finishing calculating.
---********************************************************
